...
$targetLOINCCode = filter_input(INPUT_GET, "loinc");

if (empty($targetLOINCCode)) {
    $sql = "SELECT l_id AS id, loinc, concept_name
    ...
    $sql = "SELECT l_id AS id, loinc, concept_name
            FROM vocab_master_loinc
            WHERE loinc = :loincCode
            ORDER BY loinc;";
    $stmt = $emsaDbFactory->getConnection()->prepare($sql);
    $stmt->bindValue(":loincCode", $targetLOINCCode, PDO::PARAM_STR);
}
...
if ($stmt->execute()) {
    while ($row = $stmt->fetchObject()) {
        $data[] = array(
            'id' => (int) $row->id,
            'loinc' => DisplayUtils::xSafe(trim($row->loinc), 'UTF-8', false),
            'concept_name' => DisplayUtils::xSafe(trim($row->concept_name), 'UTF-8', false)
        );
    }
}
