...
$targetCategory = filter_input(INPUT_GET, "category");

if (empty($targetCategory)) {
    $sql = "SELECT mv.id AS id, sc.label AS category, mv.concept AS concept
        FROM vocab_master_vocab mv
        INNER JOIN structure_category sc ON sc.id = mv.category
        ORDER BY sc.label, mv.concept;";
    ...
    $sql = "SELECT mv.id AS id, sc.label AS category, mv.concept AS concept
        ...
        ORDER BY sc.label, mv.concept;";
    $stmt = $emsaDbFactory->getConnection()->prepare($sql);
    $stmt->bindValue(":category", $targetCategory, PDO::PARAM_STR);
}
