...
$sql = "SELECT vmo.o_id AS id, vmo.snomed AS snomed, mv.concept AS concept 
        FROM vocab_master_organism vmo 
        LEFT JOIN vocab_master_vocab mv ON (vmo.organism = mv.id) 
        INNER JOIN vocab_master_vocab mv2 ON (vmo.snomed_category = mv2.id) 
        WHERE mv2.category = vocab_category_id('snomed_category') AND mv2.concept = 'Organism' 
...
if ($stmt->execute()) {
    while ($row = $stmt->fetchObject()) {
        $snomeds[] = array(
            ...
            'snomed' => DisplayUtils::xSafe(trim($row->snomed), 'UTF-8', false)
        );
    }
}
...
echo json_encode($snomeds, JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG);
exit;
