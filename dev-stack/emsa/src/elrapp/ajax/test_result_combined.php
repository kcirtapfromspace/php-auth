...
$sql = "SELECT id, concept || ' (Labs)' as concept, 'Labs' as test_result_type
        FROM vocab_master_vocab
        WHERE category = vocab_category_id('test_result')
        UNION ALL 
        SELECT id, concept || ' (AST)' as concept, 'AST' as test_result_type
        FROM vocab_master_vocab
        WHERE category = vocab_category_id('resist_test_result')
...
if ($stmt->execute()) {
    while ($row = $stmt->fetchObject()) {
        $testResults[] = array(
            'id' => (int) $row->id,
            'concept' => DisplayUtils::xSafe($row->concept, 'UTF-8', false)
        );
    }
}
