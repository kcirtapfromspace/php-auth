	...
	if (jsonObj.id && jsonObj.action) {
		...
		for (var i = 0; i < jsonObj.conditions.length; i++) {
			...
			$.ajax({
				...
				$.each(data, function(idx, val) {
					$("#rulemod_cmr_operator_"+(i)).append("<option value='"+val.id+"'>"+val.label+" ("+val.graphical+")</option>");
				});
			});
			...
			$.ajax({
				...
				$.each(data, function(idx, val) {
					$("#rulemod_cmr_operand_value_"+(i)).append("<option value='"+val.id+"'>"+val.concept+"</option>");
				});
			});
		...
		$("#rule_mod_cmr_dialog").dialog('option', 'buttons', {
				"Save Changes" : function() {
					$(this).dialog("close");
					handleEditSubmit($("#rule_mod_cmr_form"), targetDiv, jsonObj.callback_handler, jsonObj.callback_tbl, jsonObj.focus_id, jsonObj.callback_noun, jsonObj.callback_col);
					},
				"Cancel" : function() {
					$(this).dialog("close");
					}
...
$(".add_cmr_rule").button({
	...
	if (jsonObj.id && jsonObj.action) {
		...
		$.ajax({
			...
			$.each(data, function(idx, val) {
				$("#rulemod_cmr_operator_1").append("<option value='"+val.id+"'>"+val.label+" ("+val.graphical+")</option>");
			});
		});
		...
		$.ajax({
			...
			$.each(data, function(idx, val) {
				$("#rulemod_cmr_operand_value_1").append("<option value='"+val.id+"'>"+val.concept+"</option>");
			});
		});
...
$("#rulemod_cmr_add_condition").button({
	...
	$.ajax({
		...
		$.each(data, function(idx, val) {
			$("#rulemod_cmr_operator_"+(numConditions+1)).append("<option value='"+val.id+"'>"+val.label+" ("+val.graphical+")</option>");
		});
	});
	...
	$.ajax({
		...
		$.each(data, function(idx, val) {
			$("#rulemod_cmr_operand_value_"+(numConditions+1)).append("<option value='"+val.id+"'>"+val.concept+"</option>");
		});
	});
