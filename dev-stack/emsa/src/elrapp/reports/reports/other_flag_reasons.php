...
echo '<strong class="big_strong">&ldquo;Other&rdquo; QA Flag Reasons</strong>';
...
$sql = 'SELECT DISTINCT fc.info AS info, COUNT(fc.info) AS counter 
			...
			WHERE fc.system_message_id IN ( 
				...
				WHERE sm.message_flags & ' . EMSA_FLAG_DE_OTHER . ' != 0 
				' . $lab_sql . ' 
				...
				AND fc.system_message_flag_id = ' . EMSA_FLAG_DE_OTHER . ' 
			GROUP BY fc.info 
			ORDER BY 2 DESC, 1;';
...
<script type='text/javascript'>
    function drawChart() {
        ...
        summaryData.addColumn('string', 'Reason');
<?php
foreach ($results_array as $user_key => $user_stats) {
    if (intval($user_stats['Number of Occurrences']) > 0) {
        echo "summaryData.addRow([" . intval($user_stats['Number of Occurrences']) . ", '" . htmlentities($user_stats['Reason'], ENT_QUOTES, 'UTF-8') . "']);" . PHP_EOL;
    }
}
?>
        var summaryOptions = {
