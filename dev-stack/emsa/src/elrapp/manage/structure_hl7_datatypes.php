...
<script>
	$(function() {
        ...
        $(".delete_datatype").on("click", function(e) {
            e.preventDefault();
            var deleteAction = "<?php echo $webappBaseUrl; ?>?selected_page=6&submenu=4&cat=10&subcat=2&delete_id=" + $(this).val();
...
<h1 class="elrhdg"><span class="ui-icon ui-icon-header ui-icon-emsasettings"></span>Reporter-Specific HL7 Data Type Configuration</h1>

<div id="date_filter" style="position: absolute; z-index: 999; top: 43px; right: 15px;">
	<button id="addnew_button" title="Set up a new default HL7 data type">Add HL7 Data Type</button>
</div>
...
<?php
    ...
    $datatypeActionLabId = (int) filter_input(INPUT_GET, 'lab_id', FILTER_SANITIZE_NUMBER_INT);
    ...
    if ($datatypeAddFlag === 1) {
        try {
            $addSql = "INSERT INTO structure_hl7_valuetype (lab_id, value_type, interpret_results)
                       VALUES (:labId, :valueType, :interp);";
            $addStmt = $adminDbConn->prepare($addSql);
            $addStmt->bindValue(':labId', $datatypeActionLabId, PDO::PARAM_INT);
            $addStmt->bindValue(':valueType', $datatypeActionTypeName, PDO::PARAM_STR);
            $addStmt->bindValue(':interp', $datatypeActionInterpretResults, PDO::PARAM_BOOL);
            
            $addStmt->execute();
            
            Udoh\Emsa\Utils\DisplayUtils::drawHighlight('New reporter-specific data type added successfully!', 'ui-icon-elrsuccess');
        } catch (Throwable $e) {
            \Udoh\Emsa\Utils\ExceptionUtils::logException($e);
            Udoh\Emsa\Utils\DisplayUtils::drawError('An error occurred while attempting to add a new reporter-specific data type.');
        }
    } elseif ($datatypeDeleteId > 0) {
        try {
            $deleteSql = "DELETE FROM structure_hl7_valuetype 
                          WHERE id = :deleteId;";
            ...
            Udoh\Emsa\Utils\DisplayUtils::drawHighlight('Reporter-specific data type successfully deleted!', 'ui-icon-elrsuccess');
        } catch (Throwable $e) {
            \Udoh\Emsa\Utils\ExceptionUtils::logException($e);
            Udoh\Emsa\Utils\DisplayUtils::drawError('An error occurred while attempting to delete this reporter-specific data type.');
        }
    } elseif ($datatypeEditId > 0) {
        try {
            $updateSql = "UPDATE structure_hl7_valuetype
                          SET lab_id = :labId, value_type = :valueType, interpret_results = :interp
                          WHERE id = :editId;";
            $updateStmt = $adminDbConn->prepare($updateSql);
            $updateStmt->bindValue(':labId', $datatypeActionLabId, PDO::PARAM_INT);
            $updateStmt->bindValue(':valueType', $datatypeActionTypeName, PDO::PARAM_STR);
            $updateStmt->bindValue(':interp', $datatypeActionInterpretResults, PDO::PARAM_BOOL);
            $updateStmt->bindValue(':editId', $datatypeEditId, PDO::PARAM_INT);
            
            $updateStmt->execute();
            
            Udoh\Emsa\Utils\DisplayUtils::drawHighlight('Reporter-specific data type successfully updated!', 'ui-icon-elrsuccess');
        } catch (Throwable $e) {
            \Udoh\Emsa\Utils\ExceptionUtils::logException($e);
            Udoh\Emsa\Utils\DisplayUtils::drawError('An error occurred while attempting to update this reporter-specific data type.');
        }
    }
...
<div id="addnew_form" class="addnew_lab ui-widget ui-widget-content ui-corner-all">
    <div style="clear: both;"><span class="emsa_form_heading">Add New Reporter-Specific Data Type:</span><br><br></div>
    <form id="new_lab_form" method="GET">
        <label class="vocab_search_form2" for="new_lab_id">Reporting Facility:</label>
        <select class="ui-corner-all" name="lab_id" id="new_lab_id">
            <option value="0" selected>--</option>
        <?php
            $reportingFacilities = \Udoh\Emsa\Utils\CoreUtils::getReporterList($adminDbConn, true);
            
            if (!empty($reportingFacilities)) {
                foreach ($reportingFacilities as $rfId => $rfName) {
                    echo '<option value="' . (int) $rfId . '">' . Udoh\Emsa\Utils\DisplayUtils::xSafe((string) filter_var($rfName, FILTER_SANITIZE_STRING), "UTF-8", false) . "</option>\n";
                }
            }
        ?>
        </select>
        ...
        <br><br><button type="submit" name="new_savedatatype" id="new_savedatatype">Save New Reporter-Specific Data Type</button>
...
<?php
	...
	$currentSql = 'SELECT sv.id AS id, sv.lab_id AS lab_id, sl.ui_name AS lab_name, sv.value_type AS value_type, sv.interpret_results AS interpret_results
		FROM structure_hl7_valuetype sv
        INNER JOIN structure_labs sl ON (sv.lab_id = sl.id)
        ORDER BY sl.ui_name, sv.value_type;';
	...
	while ($row = $currentRs->fetchObject()) {
        ...
        $editLhdParams = array(
            ...
            "interpret_results" => ((bool) $row->interpret_results === true) ? 't' : 'f'
        );
        
        echo "<button class='edit_datatype' type='button' value='" . json_encode($editLhdParams) . "' title='Edit this Reporter-Specific Data Type'>Edit</button>";
        echo "<button class='delete_datatype' type='button' value='" . (int) $row->id . "' title='Permanently delete this Reporter-Specific Data Type'>Delete</button>";
        
		echo "</td>";
        echo "<td>" . \Udoh\Emsa\Utils\DisplayUtils::xSafe((string) $row->lab_name) . "</td>";
		echo "<td>" . \Udoh\Emsa\Utils\DisplayUtils::xSafe((string) $row->value_type) . "</td>";
        
        if ((bool) $row->interpret_results) {
...
<div id="confirm_delete_dialog" title="Delete this Reporter-Specific Data Type?">
...
<div id="edit_datatype_dialog" title="Edit Reporter-Specific Data Type">
    <form id="edit_modal_form" method="GET">
        <label class="vocab_search_form2" for="edit_lab_id">Reporting Facility:</label>
        <select class="ui-corner-all" name="lab_id" id="edit_lab_id">
            <option value="0" selected>--</option>
        <?php
            $reportingFacilities = \Udoh\Emsa\Utils\CoreUtils::getReporterList($adminDbConn, true);
