	...
	if (jsonObj.id && jsonObj.action) {
            ...
            (function(idx) {
                $.ajax({
                    ...
                    if (data.length) {
                        ...
                        $("#rulemod_ms_cmr_operator_"+(idx)).empty().append(operatorOptions.join(''));
                    }
                    $("#rulemod_ms_cmr_operator_"+(idx)).val(jsonObj.conditions[idx].operator);
                });
            })(i);
            ...
            (function(idx) {
                $.ajax({
                    ...
                    if (data.length) {
                        ...
                        $("#rulemod_ms_cmr_operand_value_"+(idx)).empty().append(testResultOptions.join(''));
                    }
                    $("#rulemod_ms_cmr_operand_value_"+(idx)).val(jsonObj.conditions[idx].operand);
                });
            })(i);
		}
		...
		$("#rule_mod_ms_cmr_dialog").dialog('option', 'buttons', {
				"Save Changes" : function() {
					$(this).dialog("close");
					$("#rule_mod_ms_cmr_form").trigger("submit");
					},
				"Cancel" : function() {
					$(this).dialog("close");
					}
	...
	if (jsonObj.id && jsonObj.action) {
        ...
        $.ajax({
            ...
            if (data.length) {
                ...
                $("#rulemod_ms_cmr_operator_1").empty().append(operatorOptions.join(''));
            }
        });
        ...
        $.ajax({
            ...
            if (data.length) {
                ...
                $("#rulemod_ms_cmr_operand_value_1").empty().append(testResultOptions.join(''));
            }
        });
...
$(".delete_ms_cmr_rule").button({
	...
	if (jsonObj.id) {
		var deleteRuleAction = "<?php echo $webappBaseUrl; ?>?selected_page=6&submenu=3&cat=<?php echo $navCat; ?>&subcat=<?php echo $navSubcat; ?>&rulemod_ms_cmr_action=delete&rulemod_ms_cmr_id="+jsonObj.id+"&focus_id="+jsonObj.focus_id;
...
$("#rulemod_ms_cmr_add_condition").button({
    ...
    $.ajax({
        ...
        if (data.length) {
            ...
            $("#rulemod_ms_cmr_operator_"+(numConditions+1)).empty().append(operatorOptions.join(''));
        }
    });
    ...
    $.ajax({
        ...
        if (data.length) {
            ...
            $("#rulemod_ms_cmr_operand_value_"+(numConditions+1)).empty().append(testResultOptions.join(''));
        }
    });
